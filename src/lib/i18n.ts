...
const ru: Dict = {
  ...
  fieldScope: "Scope",
  fieldCaPath: "Путь к Russian Trusted Root CA (PEM)",
  gigaNote:
    "Без сертификата возможна ошибка TLS. OAuth идёт на порт 9443, токен кэшируется, запросы сериализуются (freemium — 1 за раз).",
...
const en: Dict = {
  ...
  fieldGigaAuth: "Authorization key (Basic)",
  fieldScope: "Scope",
  fieldCaPath: "Path to Russian Trusted Root CA (PEM)",
  gigaNote:
    "Without the certificate TLS may fail. OAuth uses port 9443; the token is cached; requests are serialized (freemium — 1 at a time).",
...
const es: Dict = {
  ...
  fieldScope: "Scope",
  fieldCaPath: "Ruta al Russian Trusted Root CA (PEM)",
  gigaNote:
    "Sin el certificado puede fallar el TLS. OAuth usa el puerto 9443; el token se cachea; las solicitudes se serializan (freemium — 1 a la vez).",
