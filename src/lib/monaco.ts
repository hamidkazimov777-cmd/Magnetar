...
monaco.editor.defineTheme(MAGNETAR_THEME_DARK, {
  base: "vs-dark",
  inherit: true,
  rules: [
...
monaco.editor.defineTheme(MAGNETAR_THEME_LIGHT, {
  base: "vs",
  inherit: true,
  rules: [
...
/** Called by the theme sync in main.tsx whenever the palette flips. */
...
// Monaco 0.56 moved this out of `languages.typescript` into a top-level export.
// Semantic errors are noise without the real project graph (every import would
// be "not found"), so we keep syntax validation and drop semantic validation.
