...
### Запись 114 — 2026-08-26 — Claude — Step 2, срез 1: CSP

Закрыт зафиксированный release blocker `"csp": null`.

**Цель среза**: политика содержимого для вебвью, не ломающая приложение.
Секреты и path containment сознательно не трогались — по секретам нужно
решение владельца (см. ниже).

**Что выяснено перед работой**

- Фронтенд **не ходит в сеть напрямую**: ни одного `fetch`, `XMLHttpRequest`,
  `EventSource` или `WebSocket` во всём `src/`. Все вызовы провайдеров идут
  через Rust `invoke`. Поэтому `connect-src` затянут до `'self' ipc:`.
- Monaco-воркеры Vite собирает как обычные same-origin `.js` в `dist/assets/`,
  blob не требуется — но `worker-src` оставлен `'self' blob:` на случай смены
  режима сборки.
- Studio рисует сгенерированные ассеты по удалённому URL
  (`StudioView.tsx:300`), поэтому `img-src`/`media-src` вынужденно содержат
  `https:`. После Шага 13 (ассеты на диск) это можно убрать.

**Изменения**

- `src-tauri/tauri.conf.json` — политика: `default-src 'self'`,
  `script-src 'self'` (без `unsafe-inline`/`unsafe-eval`),
  `style-src 'self' 'unsafe-inline'` (Monaco инжектит стили),
  `connect-src 'self' ipc: http://ipc.localhost`, `object-src 'none'`,
  `frame-ancestors 'none'`, `form-action 'none'`.
- `src-tauri/src/lib.rs` — `mod config_tests`: четыре теста читают
  `tauri.conf.json` через `include_str!` и падают, если CSP исчез, если в
  `script-src` появился `unsafe-*` или `*`, если `connect-src` пустили в
  интернет, или если `https:` расползся на исполняемые директивы.

**Как проверено (не рассуждением, а прогоном)**

Собранный `dist/` отдан локальным сервером с этой же политикой в виде
настоящего заголовка `Content-Security-Policy`, приложение открыто в браузере:

- welcome-экран и весь shell (activity bar, дерево, редактор, статус-бар)
  отрисовались полностью — шрифты, стили, логотип на месте;
- **ноль нарушений CSP в консоли**;
- прямые пробы в контексте страницы: инлайн-скрипт — заблокирован,
  кросс-origin скрипт — заблокирован, `fetch` на сторонний домен —
  заблокирован, same-origin воркер — разрешён, blob-воркер — разрешён,
  инжект `<style>` — разрешён.

Про `eval` вывод не делаю: консоль девтулзов освобождена от CSP, её результат
здесь не доказательство. Отсутствие `'unsafe-eval'` гарантируется тестом.

Валидность формы CSP для Tauri подтверждается тем, что `generate_context!`
разбирает `tauri.conf.json` на этапе компиляции, а `cargo check --all-targets`
прошёл без единого warning.

**Найдено попутно, НЕ моё и не исправлено**

`Composer.tsx:85` вызывает `getCurrentWebview()`, который читает
`window.__TAURI_INTERNALS__.metadata`. Вне Tauri это бросает исключение и
роняет панель агента в error boundary. Внутри реального приложения работает.
Это существовавшая раньше хрупкость (в `api.ts` для этого есть guard
`HAS_BACKEND`, но `Composer` и `Splash` им не пользуются). Она делает любую
проверку в браузере шумной. Отдельная задача.

**Гейты**: typecheck OK, Vitest 117/117, Rust 35/35, `cargo check --all-targets`
0 warnings, build OK, smoke 4/4.

**Ограничения**

- Monaco под CSP не поднимался целиком: без Tauri нельзя открыть файл.
  Проверено косвенно — воркеры и инжект стилей разрешены прямой пробой.
- Проверка шла в браузере с тем же заголовком, а не в реальном WKWebView.
  Tauri-специфичные протоколы (`ipc:`, asset) в браузере не проверить.
  Первый запуск собранного `.app` должен это подтвердить.

**НУЖНО РЕШЕНИЕ ВЛАДЕЛЬЦА — секреты.** Шаг 2 требует Keychain-only, но
`keychain.rs` уводит ключи оттуда намеренно и по прямой просьбе владельца:
macOS привязывает ACL к подписи кода, поэтому каждая пересборка спрашивала
пароль. Стабильная подпись появляется только на Шаге 15. Варианты: (а) оставить
0600-файл до подписи; (б) Keychain сразу и терпеть запросы; (в) переключатель.
До ответа секреты не трогаю.

**Следующий шаг**: сужение Tauri capabilities (`fs:default` слишком широк) и
canonical path containment в Rust с тестами на побег через `..`, симлинк и
абсолютный путь.

### Запись 115 — 2026-08-26 — Claude — Step 2, срез 2: примитив containment путей

**Поправка к записи 114 и к устному выводу**: я утверждал, что локальной
подписи «Magnetar Dev» на машине нет, опираясь на `security find-identity -v`.
Команда без `-p codesigning` этот самоподписанный сертификат не показывает.
Сертификат существует (`04DFE2CE…`, 1 valid identity). Значит инфраструктура
стабильной подписи уже на месте, и причина ухода ключей из Keychain снимается
без Apple Developer Program. Владелец пошёл по пути «делать сейчас», поэтому
Keychain-миграция остаётся в Шаге 2.

Отдельно: `npm run tauri build` не накладывает эту подпись сам — в
`tauri.conf.json` нет `signingIdentity`, подпись ставит только
`scripts/sign-app.sh`. Это дыра в процессе: забыл второй скрипт — получил
нестабильную подпись и запросы пароля обратно. Кандидат на починку с фолбэком
для машин без сертификата (CI).

**Что сделано в этом коммите**

`src-tauri/src/paths.rs` — примитив, отвечающий на один вопрос: куда на самом
деле указывает путь и внутри ли он workspace root.

- `lexically_normalize` схлопывает `.` и `..` без обращения к ФС — потому что
  цель может ещё не существовать (`write_file` создаёт файлы, а `canonicalize`
  на несуществующем падает);
- `resolve_existing_prefix` канонизирует самый глубокий существующий предок и
  приклеивает остаток — именно это ловит симлинк: ссылка внутри workspace,
  ведущая наружу, разрешается в «наружу»;
- `locate` возвращает `Inside`/`Outside`, `require_inside` отказывает с
  сообщением, называющим **разрешённое** место, а не то, как путь написали.

Сознательное решение: **containment, а не забор**. Системный промпт Magnetar
прямо разрешает агенту работать вне открытой папки по просьбе пользователя —
эта формулировка появилась после того, как ранняя версия прочитала root как
стену и отказалась создать папку на Рабочем столе. Поэтому модуль только
устанавливает факт, а решение про внешние пути остаётся за явным грантом.

**Тесты (11 новых, Rust 35 → 46)**: обычные пути, несуществующий файл, `..`
наружу, `..` возвращающийся внутрь, симлинк-файл наружу, симлинк-каталог
наружу, симлинк внутри, сосед с общим префиксом имени (`/work/project-old`
против `/work/project` — классическая ошибка строкового сравнения), сам root,
абсолютный путь снаружи, пустой путь и несуществующий root.

Один тест по ходу поймал не баг кода, а мою неверную ожидаемость: `/etc` на
macOS — симлинк на `/private/etc`, и код правильно разрешил его до реального
места. Утверждение исправлено, комментарий объясняет почему.

**Известное ограничение, зафиксированное явно**

Модуль пока не вызывается ни одним инструментом и помечен `#![allow(dead_code)]`
с комментарием. Это сделано осознанно: подключение требует бэкенд-хранимого
workspace root, записи грантов и правки политики подтверждений — смешивать это
с примитивом в одном патче значит получить нечитаемый коммит. Следующий срез
подключает.

**Гейты**: typecheck OK, Vitest 117/117, Rust 46/46, `cargo check --all-targets`
0 warnings, build OK, smoke 4/4.

**Следующий шаг**: бэкенд-хранимый workspace root + команда его установки,
подключение `require_inside` к файловым инструментам, запись грантов для
внешних путей. Затем сужение capabilities (`fs:default`) и Keychain-миграция.

### Запись 116 — 2026-08-26 — Claude — Step 2, срез 3: containment включён

Примитив из записи 115 подключён; `#![allow(dead_code)]` снят, мёртвого кода
не осталось.

**Где теперь проходит граница**

Workspace root хранится в Rust (`paths::set_workspace_root`), а не в store.
Это принципиально: вебвью — та сторона, которую компрометируют, и граница,
которую она может подвинуть, границей не является. Фронтенд её только
сообщает при открытии и закрытии папки.

`paths::authorize` возвращает `Allowed` или `NeedsGrant`. Все файловые
команды (`tool_read_file`, `tool_list_dir`, `tool_grep`, `tool_write_file`,
`tool_delete_file`, `tool_edit_file`, `tool_attach_file`, `extract_pdf_text`,
`editor_read_file`) проходят через `ensure_allowed` и получают уже
разрешённый `PathBuf`.

**Подтверждение рисует бэкенд.** На `NeedsGrant` Rust показывает нативный
диалог `tauri-plugin-dialog` с реальным местом назначения и ждёт ответа через
`oneshot`. Фронтенд его не рисует, не может пропустить и не может ответить за
пользователя. Это и есть разница между «пользователь подтвердил» и
«страница сказала, что пользователь подтвердил». Гранты живут только в памяти
процесса и сбрасываются при смене папки — грант, переживший перезапуск, это
разрешение, о котором никто не помнит. Грант на каталог покрывает вложенное,
чтобы не спрашивать по файлу.

Попутно исправлено: `tool_grep` без пути искал в рабочем каталоге процесса —
то есть где угодно, откуда запустили приложение. Теперь `"."` разрешается
относительно открытого проекта. Это одновременно и containment, и правильное
поведение по умолчанию.

**Тесты**: Rust 46 → 50 (root не задан; внутри разрешено, снаружи спрашивает;
грант на каталог покрывает вложенное, но не соседа с похожим именем; смена
папки забывает все гранты). Frontend 117 → 118: store обязан сообщать
бэкенду и об открытии, и о закрытии папки.

`require_inside` удалён как вытесненный `authorize`; его полезное свойство —
сообщение называет **разрешённое** место, а не написание — вынесено в
`paths::outside_message` и покрыто тестом на симлинке.

**Гейты**: typecheck OK, Vitest 118/118, Rust 50/50,
`cargo check --all-targets` 0 warnings, build OK, smoke 4/4.

**Ограничения — читать обязательно**

1. **`tool_run_bash` под политику НЕ подпадает.** Команда получает строку и
   выполняет её. Всё, что containment запрещает через файловый инструмент,
   по-прежнему достижимо через shell. Пока bash не приведён под ту же
   политику, containment — это существенное сужение, а не закрытая граница.
   Это следующий срез.
2. Нативный диалог автотестами не проверяется — нужен запущенный app и
   человек. Логика решения (`authorize`) покрыта, показ диалога — нет.
3. Возможен двойной вопрос: фронтенд уже имеет свой confirm для записи, и на
   внешнем пути пользователь увидит сначала его, потом нативный. Устранится,
   когда фронтендовый confirm перестанет дублировать бэкендовый.

**Следующий шаг**: bash под ту же политику (cwd, containment, аудит), затем
сужение `fs:default` в capabilities, затем Keychain-миграция.

### Запись 117 — 2026-08-26 — Claude — Step 2, срез 4: bash под политику и аудит

Закрыта дыра из записи 116: shell обходил containment.

**Что сделано**

`src-tauri/src/audit.rs` — append-only локальный журнал: `audit.log` в каталоге
данных приложения, режим 0600, ротация на 2 МБ с одним поколением назад.
Одна строка = одно событие в JSON: время, вид, cwd, команда, исход.

`tool_run_bash` и `git_exec` теперь разрешают свой рабочий каталог через тот же
`ensure_allowed`, что и файловые инструменты. Без указанного cwd команда
раньше наследовала рабочий каталог процесса — то есть выполнялась там, откуда
запустили приложение. Теперь `"."` — это открытый проект.

`git_exec` добавлен намеренно: он принимает произвольные аргументы, включая
`git config --global` и `git push`, то есть пишет за пределы репозитория.

**Редакция секретов в Rust**, без добавления зависимости `regex`: маркеры
(`authorization`, `api_key`, `token`, `password`, `client_id`, `bearer`…) плюс
распознавание формы ключа (`sk-…`, `ghp_…`, `github_pat_…`) от 12 символов
тела. Журнал команд — ровно то место, куда пользователь вставляет ключ руками,
поэтому редактор намеренно грубый: переусердствовать значит потерять контекст,
недоусердствовать значит записать ключ в файл.

**Тесты**: Rust 50 → 57. Вставленный ключ в `curl -H 'Authorization: Bearer
sk-…'` не доходит до журнала, но `curl` и URL остаются читаемыми; присваивания
и флаги (`API_KEY=…`, `--password=…`, `--client_id …`) покрыты; голый ключ без
маркера ловится; обычные команды (`git status`, `cargo test --all`) проходят
без изменений; короткий двойник `sk-1.txt` ключом не считается; маркер без
значения не трогается; запись до `init` не падает.

**Гейты**: typecheck OK, Vitest 118/118, Rust 57/57,
`cargo check --all-targets` 0 warnings, build OK, smoke 4/4.

**Ограничения**

1. Containment для shell — это где команда **начинается**, а не куда дойдёт.
   Команда свободна сделать `cd` куда угодно. Это принципиально: понять, что
   тронет `make deploy`, можно только выполнив его. Остаточный охват закрыт
   журналом, а не запретом, и в `SECURITY.md` записан именно так.
2. UI для чтения журнала нет. Пока: `cat "$HOME/Library/Application
   Support/com.hamidkazimov.magnetar/audit.log"`.
3. Журнал не проверялся на реально запущенном приложении — `audit::init`
   вызывается из `setup`, тесты покрывают редакцию и то, что запись без
   каталога не падает.

**Следующий шаг**: сужение `fs:default` в capabilities, затем Keychain-миграция,
затем trust/read-only режимы и авторизация остальных команд.

### Запись 118 — 2026-08-26 — Claude — Step 2, срез 5: capabilities и единая политика файлов

**Поправка к записям 114 и 116.** Я писал, что `fs:default` «слишком широк».
Это неверно. `fs:default` даёт доступ только к каталогам самого приложения
(AppConfig/AppData/AppCache/AppLog). Произвольные файлы читались потому, что
`tauri-plugin-dialog` при выборе файла добавляет его путь в fs-scope в рантайме
(`s.allow_file(&path)` в `commands.rs` плагина).

Настоящая проблема оказалась другой и хуже: это **вторая политика рядом с
containment**, решающая тот же вопрос по другим правилам и без записи в аудит.
И она не покрывает drag-and-drop — перетаскивание не есть выбор, поэтому
`readFile` на бро́шенный файл должен был падать.

**Найденный баг**: `ingestPaths` в `Composer.tsx` используется и для picker, и
для drop, но `readFile` обёрнут в try только для PDF-ветки. Перетаскивание
картинки, судя по коду, молча не добавляло вложение. На живом приложении не
воспроизводил — вывод из чтения кода и разрешений плагина.

**Что сделано**

- `read_file_base64` — Rust-команда чтения вложения через `ensure_allowed`,
  с лимитом 25 МБ и внятной ошибкой с размером;
- `pick_attachments` — системный файловый диалог открывает **бэкенд**, а
  выбранные пути сразу получают грант. Выбор файла и есть авторизация: человек,
  выбравший `~/Desktop/photo.png` в системном диалоге, сказал это яснее любого
  второго вопроса. Грант выдаётся на файл, а не на папку — выбор одной картинки
  не разрешает читать всё, что лежит рядом;
- фронтенд (`Composer.tsx`, `StudioView.tsx`) больше не импортирует
  `@tauri-apps/plugin-fs`; две копии `arrayBufferToBase64` удалены;
- `fs:default` убран из capabilities, `tauri_plugin_fs::init()` снят с
  регистрации, зависимость удалена из `package.json` и `Cargo.toml`;
- `opener:allow-open-url` убран как дубль `opener:default`.

Побочный эффект, который и был целью: теперь у чтения файлов **одна** политика,
и она пишет в аудит.

**Тесты**: Rust 57 → 58 — новый `config_tests` падает, если в capabilities
появится любое `fs:`, и проверяет, что `dialog:default` остался (нужен для
выбора папки и экспорта памяти).

**Гейты**: typecheck OK, Vitest 118/118, Rust 58/58,
`cargo check --all-targets` 0 warnings, build OK, smoke 4/4.

**Ограничения**

- Ни picker, ни drag-and-drop не проверены на живом приложении — нужен
  запущенный Tauri. Проверка на первом запуске собранного `.app` обязательна:
  это единственный срез, который менял пользовательский поток, а не только
  политику.
- Изоляция встроенного браузерного окна (`SubscriptionsView`) не пересмотрена.

**Следующий шаг**: Keychain-миграция, затем trust/read-only режимы и
авторизация остальных команд.

### Запись 119 — 2026-08-26 — Claude — Step 2, срез 6: секреты вернулись в Keychain

**Что изменено**

`keychain.rs` инвертирован. Раньше модуль уводил ключи **из** Keychain в файл;
теперь Keychain — основное хранилище, а `secrets.json` остался только
dev-фолбэком.

- `set_key` пишет в Keychain; при успехе удаляет копию из файла, чтобы не
  осталось устаревшего дубликата;
- если Keychain отказал: в debug-сборке — фолбэк в файл с записью в stderr;
  в release — **ошибка**, ключ на диск в открытом виде не пишется никогда.
  Молча понизить защиту чужих учётных данных хуже, чем отказать;
- `get_key` сначала спрашивает Keychain, затем читает старый файл и
  **переносит** ключ в Keychain, удаляя plaintext-копию только после успешной
  записи;
- чтение старого файла разрешено во всех сборках — иначе существующие ключи
  невозможно спасти.

Политика вынесена в `plaintext_fallback_allowed()` и чистую функцию `decide`,
чтобы её можно было проверить тестом, а не читать `cfg` по всем путям записи.

**Исправлена неправда в интерфейсе.** Settings показывал «Ключ сохранён» через
строку `keyInKeychain` всё время, пока ключи лежали в файле. Теперь команда
`key_storage` возвращает реальное место, и UI различает Keychain, файл
(с пометкой «только для отладки») и отсутствие ключа. Строки добавлены в ru/en/es.

**Тесты**: Rust 58 → 62. Release не пишет в открытом виде; рабочий Keychain
предпочитается всегда; фолбэк действительно завязан на `debug_assertions`;
сообщение об отказе не цитирует ключ.

Удалён мёртвый `migrate_from_keychain` — остаток обратной миграции.

**Гейты**: typecheck OK, Vitest 118/118, Rust 62/62,
`cargo check --all-targets` 0 warnings, build OK, smoke 4/4.

**Что увидит владелец при следующем запуске**

На машине есть `secrets.json` (813 байт, 0600). При первом чтении каждого ключа
новый код перенесёт его в Keychain и уберёт из файла. macOS может один раз
спросить пароль на каждый ключ — и в этот раз «Always Allow» должен прилипнуть,
потому что подпись стабильна. Обязательное условие: сборка подписана
`bash scripts/sign-app.sh` после `npm run tauri build`.

**Ограничения**

- Реальный Keychain автотестами не проверяется: нужен запущенный подписанный
  билд и человек. Покрыта политика решения, не системный вызов.
- Миграция на живых ключах не выполнялась. Файл не удаляется целиком — он
  очищается по мере переноса ключей; пустой JSON остаётся.
- `npm run tauri build` по-прежнему не подписывает сам (нет `signingIdentity`
  в конфиге). Если забыть `sign-app.sh`, подпись нестабильна и запросы пароля
  вернутся. Это следующий кандидат на починку.

**Следующий шаг**: `signingIdentity` в конфиг с фолбэком, затем trust/read-only
режимы и авторизация остальных Tauri-команд.

### Запись 120 — 2026-08-26 — Claude — Step 2, срез 7: read-only и классификация команд

**Сборка и подпись — одна команда** (коммит `0aa611b`). `npm run build:app`
делает `tauri build` и `sign-app.sh`. Забыть вторую половину больше нельзя.
`signingIdentity` в `tauri.conf.json` намеренно НЕ прописан: на машине без
этого сертификата (CI) сборка бы сломалась.

**Режим «только чтение»** — `src-tauri/src/policy.rs`.

Containment отвечает на вопрос «где», policy — на вопрос «можно ли». Это
разные вопросы: запись внутри workspace всё равно запись, и есть ситуации
(разбор чужого репозитория, знакомство с агентом до того, как ему доверяешь),
где смысл именно в том, чтобы читать всё и не менять ничего.

- `decide(access, read_only)` — чистая функция, тестируется без глобального
  состояния;
- выполнение запрещается **вместе** с записью, сознательно: команда непрозрачна,
  и считать `sh build.sh` чтением значит превратить режим в обещание, которое
  приложение не может сдержать;
- по умолчанию выключен и не персистится. Контрол, переживающий перезапуск,
  становится настройкой, которую никто не замечает, вместо выбора, который
  кто-то делает.

Переключатель в статус-баре, но настоящий выключатель в Rust: store хранит
только зеркало для UI. Компрометация страницы не снимает режим.

**Классификация всех команд.** Каждая файловая и процессная команда объявляет
`Access::Read`, `Write` или `Execute` — 12 точек. Изначально `Read` нигде не
использовался, и компилятор это заметил; я не стал удалять вариант, а проставил
его на читающих командах. Классификация, которая появляется только там, где
что-то запрещено, — не политика, а россыпь частных случаев.

**Тесты**: Rust 62 → 67, frontend 118 → 119 (store обязан сообщать бэкенду).

**Проверено в интерфейсе**: собранный `dist` отдан с реальным CSP, статус-бар
показывает «Изменения разрешены», клик переключает на «Только чтение» с
предупреждающей подсветкой. Скриншоты сняты.

**Гейты**: typecheck OK, Vitest 119/119, Rust 67/67,
`cargo check --all-targets` 0 warnings, build OK, smoke 4/4.

**Ограничения**

- Фактический отказ команды в read-only на живом приложении не проверялся:
  вне Tauri команды не вызываются. Покрыто логикой и тестами, не сквозным
  прогоном.
- **Repository trust не реализован.** Открытие чужого репозитория даёт ему те
  же права, что и своей работе. Это последний невыполненный пункт Шага 2 после
  secret scanning.
- Сеть не является объявленной capability: `Access` не покрывает исходящие
  запросы провайдеров.

**Следующий шаг**: repository trust (по умолчанию restricted для незнакомой
папки) и secret scanning. После этого Шаг 2 закрыт.

### Запись 121 — 2026-08-26 — Claude — Step 2, срез 8: repository trust

Trust встроен в тот же `policy.rs`, чтобы режим не был размазан по модулям.

**Правило**: незнакомая папка читается, но не меняется, и команды в ней не
запускаются. Открыть папку — не то же самое, что поручиться за её содержимое:
репозиторий несёт build-скрипты, определения задач и конфигурацию инструментов,
которые срабатывают, как только к ним прикоснулись, а склонировать чужой проект,
чтобы почитать, — обычное дело.

- `decide(access, read_only, trusted)` — по-прежнему чистая функция;
- при совпадении обоих запретов сообщается **read-only**: пользователь только
  что сам его выбрал, значит это и есть объяснение того, что он видит. Сказать
  про доверие означало бы отправить его чинить не то;
- доверие помнится по канонизированному пути в `trusted-roots.json` (0600).
  Канонизация обязательна: папка через симлинк — та же папка, и доверять ей
  дважды под двумя именами значит получить список, спорящий сам с собой;
- спрашивать каждое утро про проект, в котором человек работает ежедневно, —
  это способ превратить запрос в то, что прокликивают не читая.

Фронтенд: баннер в `Workspace.tsx` с кнопкой «Доверять этой папке», состояние
в workspace-срезе store, строки в ru/en/es. Store спрашивает бэкенд при
открытии папки, а не предполагает.

**Тесты**: Rust 67 → 73, frontend 119 → 120. Незнакомая папка читается, но не
меняется и не исполняет; доверие одной папке ничего не говорит о другой;
доверие без открытой папки — ошибка, а не тихий успех; при двух запретах
сообщается read-only. Тесты, трогающие глобальное состояние, сериализованы.

**Гейты**: typecheck OK, Vitest 120/120, Rust 73/73,
`cargo check --all-targets` 0 warnings, build OK, smoke 4/4.

**Ограничения**

- Сам баннер визуально не проверен: вне Tauri папка не открывается и
  `workspaceTrusted()` возвращает true. Проверена только неизменность
  раскладки после правки `Workspace.tsx`.
- **Существующие проекты станут недоверенными** при первом запуске новой
  сборки — файла `trusted-roots.json` ещё нет. Это ожидаемо, но владелец
  увидит баннер на своём же проекте и должен нажать кнопку один раз.

**Осталось до конца Шага 2**: secret scanning, изоляция встроенного браузера,
предупреждения о prompt injection, сеть как объявленная capability.

### Запись 122 — 2026-08-26 — Claude — ШАГ 2 ЗАВЕРШЁН

Последние три среза и итог шага.

**Secret scanning** (`0062b52`). `scripts/scan-secrets.mjs` проверяет рабочее
дерево и, когда он есть, собранный бандл. Два места отказывают по-разному:
дерево — куда ключ вставляют «просто проверить» и коммитят; бандл — куда его
встраивает шаг сборки, который никто не смотрел. Плюс проверка, что файл с
учётными данными не отслеживается git.

Сканер намеренно узкий: префиксы провайдерских ключей, идентификаторы облачных
ключей, заголовки приватных ключей. Сканер, выдающий сотню «может быть», — это
сканер, который перестают читать. Совпадения печатаются замаскированными.
Проверен подсадкой ключа OpenAI-формы и AWS key id: оба найдены, значения
замаскированы, код возврата 1. Встроен в smoke (теперь 5/5) и в CI.

**Prompt injection** (`96c0bda`). `src/lib/injection.ts` + правило в
`AGENT_SYSTEM`. Всё, что возвращает инструмент, приходит в тот же разговор, что
и слова пользователя, и ничем не помечено. README со словами «игнорируй
предыдущие инструкции» структурно неотличим от того же от пользователя.

Обрамление ставится в `executeTool`, а не по местам вызова, чтобы новый
инструмент не мог его тихо обойти, и только для инструментов, чей вывод
приходит извне разговора. Предупреждение **перед** содержимым: модель,
прочитавшая три тысячи строк инструкций, предупреждена слишком поздно.
Содержимое никогда не скрывается — агент, не видящий находку, не может о ней
доложить. Ложные срабатывания — часть тестов.

**Сеть и изоляция браузера** (этот коммит). `build_provider` — единственное
место сборки адаптеров, поэтому каждый достигнутый хост пишется в аудит один
раз за прогон: только хост и порт, без пути и query, потому что URL провайдера
регулярно несёт ключ в query. Гейтить сеть в BYOK-приложении нельзя: локальная
модель на странном порту так же легитимна, как известный провайдер. Честно
можно дать видимость.

Изоляция встроенного браузера: Tauri v2 ограничивает capabilities по метке
окна, а в файле указана только `main`. Окна подписок не наследуют ничего,
удалённым источникам IPC не выдаётся. Тест закрепляет и то и другое — отказ
здесь был бы тихим и полным.

**ИТОГ ШАГА 2**

Сделано: CSP по принципу deny-by-default; containment путей в бэкенде с
грантами через нативный диалог; shell и git под тем же гейтом с аудитом; одна
политика доступа к файлам и удалённый fs-capability; секреты в Keychain, в
release запись открытым текстом невозможна; read-only; repository trust;
secret scanning в smoke и CI; обрамление prompt injection; видимость исходящих
хостов; сборка и подпись одной командой.

Тесты: Rust 20 → 76, frontend 13 → 130.

**Что осознанно не закрыто и почему**

1. Security review на чистой системе — это релизный гейт Шага 14, не Шага 2.
2. Часть контролей проверена юнит-тестами, но не сквозным прогоном: нативный
   диалог гранта, миграция Keychain, баннер доверия, picker и drag-and-drop
   вложений, фактический отказ в read-only. Всем нужен запущенный подписанный
   билд и человек. Список вынесен в `docs/RELEASE_CHECKLIST.md` отдельным
   блоком; провал любого пункта — блокер релиза.

**Следующий шаг — ШАГ 3**: memory_facts и decisions как единственный источник
истины, миграция legacy prose-полей, миграции схемы и внешние ключи, очистка
данных удалённого проекта, backup/export/import, проверка целостности и
восстановление.

### Запись 123 — 2026-08-26 — Claude — два дефекта, найденных первым живым запуском

Владелец собрал и открыл приложение. Ничего не спросило — и это оказалось
частично правильным поведением, частично моей ошибкой.

**Что сработало как задумано.** Миграция ключей прошла молча и это нормально:
macOS не спрашивает пароль при *создании* новой записи Keychain, а чтение
записи, созданной той же подписью, не спрашивает тем более. Проверено фактами:
в `secrets.json` осталось 0 ключей из 7, записи в Keychain под сервисом
`com.hamidkazimov.magnetar` есть, `audit.log` создан с режимом 600, 22 строки,
неотредактированных ключей в нём нет.

**Дефект 1 — containment и trust были мертвы после перезапуска.**

`workspaceRoot` персистится в localStorage и возвращается сам, но
`api.setWorkspaceRoot` вызывается только из действий пользователя
(WelcomeView, ExplorerPanel, agent new_project). После рестарта бэкенд считал,
что папка не открыта: `paths::workspace_root()` = None (containment выключен),
`policy::set_workspace(None)` → `trusted()` = true (баннера нет).

То есть весь Шаг 2 в части путей и доверия не работал при обычном запуске.
Именно поэтому владелец не увидел баннер.

Исправлено: `adoptRestoredWorkspace()` в workspace-срезе, вызывается в конце
`hydrate`. Отдельное действие, а не переиспользование `setWorkspaceRoot`,
потому что восстановление — не «открытие папки» и не должно трогать
`recentFolders`.

**Дефект 2 — редактор аудита ел имена хостов.**

В журнале обнаружился хост `api.token[REDACTED]`. Причина: маркер совпадал
внутри слова. `git checkout secrets.ts` пострадал бы так же. Это разрушает
ценность записи и ничего не защищает.

Исправлено: маркер считается маркером, только если за ним следует разделитель.
Границу проверяю **только справа**: требование границы слева перестало бы
ловить `GITHUB_TOKEN=`, то есть ровно ту форму, ради которой всё и делается.

**Тесты**: Rust 76 → 78, frontend 130 → 131. Оба дефекта закреплены тестами с
комментарием, что их нашёл живой запуск.

**Гейты**: typecheck OK, Vitest 131/131, Rust 78/78, 0 warnings, build OK,
smoke 5/5.

**Вывод для протокола**: юнит-тесты не поймали ни один из этих дефектов, потому
что оба живут на стыке «персистентное состояние ↔ запуск процесса». Блок в
`docs/RELEASE_CHECKLIST.md` про проверку на подписанной сборке остаётся
обязательным — он уже окупился.

### Запись 124 — 2026-08-26 — Claude — drop приравнен к выбору файла

Владелец прогнал первую подписанную сборку и отчитался: 7 запросов пароля с
«Всегда разрешать», доверие папке нажато, скрепка работает, **перетаскивание
работает, но всплывает нативное окно «Work outside the project folder?»**.

Окно было формально правильным и по существу лишним. Перетаскивание файла —
такое же явное заявление намерения, как выбор в диалоге. Для picker я сделал
так, что диалог открывает бэкенд и потому сразу выдаёт грант; для drop бэкенд
не видел ничего, а фронтенд просто просил прочитать путь — с точки зрения Rust
неотличимо от произвольного пути. Пользователя просили подтвердить его же
собственный жест.

Исправлено в `lib.rs`: `.on_window_event()` ловит
`WindowEvent::DragDrop(DragDropEvent::Drop { paths, .. })` и выдаёт
`paths::grant` на каждый путь. Ключевое — грант выдаётся, **только когда drop
видит бэкенд**. Сделать это по сообщению фронтенда было бы не то же самое:
это был бы путь, о котором вебвью *утверждает*, что его перетащили.

**Гейты**: typecheck OK, Vitest 131/131, Rust 78/78, 0 warnings, build OK,
smoke 5/5.

**Открытый вопрос по паролям.** Первая сборка (22:51) создала записи в Keychain
молча — создание не спрашивает. Вторая (23:02) при чтении спросила 7 раз, и
владелец везде выбрал «Всегда разрешать». Теория подписи говорит, что ACL
привязан к designated requirement (идентификатор + сертификат), который между
пересборками стабилен, поэтому следующая сборка спрашивать не должна. Но это
пока теория: проверкой будет следующий запуск. Если спросит снова — значит ACL
привязан к cdhash конкретного бинарника, и нужен другой подход.

### Запись 125 — 2026-08-26 — Claude — один ключевой item вместо семи

Владелец прогнал сборку 23:16 и отчитался: пароли спросили снова (7 раз),
баннера доверия не было (папка уже доверена — верно), перетаскивание работает
без окна (исправление подтверждено), **а после закрытия и повторного открытия
той же сборки пароль уже не спрашивали**.

**Диагноз.** `codesign -d -r-` показывает designated requirement
`identifier "com.hamidkazimov.magnetar" and certificate leaf = H"04dfe2ce…"` —
на основе сертификата, без хэша бинарника. По теории пересборка тем же
сертификатом не должна переспрашивать. macOS переспрашивает. Значит ACL
фактически привязан к конкретной сборке; моя прежняя формулировка была
неверной. Но в пределах одной сборки разрешение держится — это подтверждено
повторным открытием.

**Что сделано.** Семь запросов — это семь отдельных записей Keychain: macOS
спрашивает про каждую. Ключи объединены в **один item** (`__magnetar_keys__`),
читаемый один раз за запуск и кешируемый. Защита та же — одна зашифрованная
запись Keychain в обоих случаях, — но прерывание падает с «раз на ключ» до
«раз на запуск».

Миграция безопасная: старая запись на подключение удаляется **только после**
успешной записи объединённого item. Удаление до неудачной записи потеряло бы
ключ.

**Тесты**: Rust 78 → 80. Слияние ключа не теряет остальные (отказ этого рода
выглядел бы как «приложение забыло провайдера»), и имя объединённого item не
может столкнуться с id подключения (UUID).

**Гейты**: typecheck OK, Vitest 131/131, Rust 80/80, 0 warnings, build OK,
smoke 5/5.

**Ожидание после следующей пересборки**: один запрос пароля вместо семи.
Проверяется живым запуском.

### Запись 126 — 2026-08-27 — Claude — ШАГ 3 ЗАВЕРШЁН

**Что было не так на самом деле.** «Двойная запись памяти» оказалась тройной, и
хуже: расходились не копии, а ответы. Агенту показывались факты с provenance;
обычному чату — старые prose-поля, отрендеренные вторым независимым куском кода
в `handoff.ts`; мосту подписок — третьим. Один и тот же вопрос в «Обсуждении» и
в «Агенте» строился из разной памяти, и только у одного из них можно было
спросить, откуда он это взял.

**Срез 1 (`bf4c76f`) — один рендерер.** `buildMemorySection` — единственный.
`buildProjectMemory` = эта секция плюс инструкции, нужные только агенту с
инструментами. Принимает не сессию, а два поля, которые реально использует, —
чтобы мост подписок не выдумывал сессию ради вызова.

Второй писатель убран: `maybeExtractProjectBrain` дописывал prose в
`techStack`/`architectureNotes`/`codingStandards` — без границ роста, без
provenance, без возможности проверки. Теперь пишет факты и решения, помеченные
как выведенные моделью из разговора, с дедупликацией по similarity, иначе факт
получал копию на каждую суммаризацию.

**Срез 2 (`0863289`) — схема владеет связью.** Удаление проекта было мягким:
одна строка помечалась, а факты, решения, дивергенции, предложения, задачи и
хронология оставались навсегда — интерфейса, который бы их показал, нет, и
который бы их удалил, тоже. Пользователя спросили «удалить проект?», он ответил
да.

Миграция v2 перестраивает шесть таблиц с `FOREIGN KEY … ON DELETE CASCADE`
документированным способом (новая таблица → копия → drop → rename), в одной
транзакции, с выключенными FK на время подмены и `foreign_key_check` после.
DDL новой таблицы **выводится из старой**, а не перепечатывается: перепечатать
список колонок — это как молча потерять одну.

Существующий тест сразу поймал изменение: он писал решение для несуществующего
проекта, чтобы доказать изоляцию. Ограничение отказало — правильно.

**Срез 3 (`3a9054a`) — снимок, копия, проверка.** Экспорт был без импорта — это
половина резервной копии, причём та половина, которая успокаивает и ничего не
делает. Импорт аддитивный, сообщает, что пропустил, и **никогда не переносит
статус проверки**: «verified» был верен на другой машине, в другое время, на
другом срезе кода.

Копия базы — `VACUUM INTO`, а не копирование файла: база в WAL, и копия
`.sqlite` может не увидеть закоммиченное или поймать запись на середине.
Перезапись существующего файла запрещена — копия, тихо заменяющая предыдущую,
это одна копия, а не две. Проверка целостности только сообщает: автоматическая
починка превращает поправимую проблему в окончательную.

Обе кнопки в панели проекта, а не в настройках: человек стоит именно здесь,
когда беспокоится за свою память.

**Срез 4 (`e8d7969`) — граф знаний убран из интерфейса.** Правило самого плана:
оставить, только если реально улучшает retrieval. Retrieval был
`recentText.includes(title)` по трём последним сообщениям. Плюс третий фоновый
писатель, добывающий третье представление того же разговора — без provenance и
без статуса проверки. Таблицы, данные и команды бэкенда не тронуты: это решение
о том, что показывать человеку, а не повод выбрасывать его данные.

**Срез 5 (`6dc92a5`) — вложения переживают перезапуск.** Колонка `attachments` в
таблице сообщений существовала и никогда не заполнялась. Метаданные — в строку,
байты — в файл в каталоге данных приложения. Не base64 в колонку: строка
сообщения читается при каждом запуске и каждом рендере, и мегабайты в ней
оплачивались бы постоянно ради картинки, нужной в момент прокрутки до неё.
Байты адресуются **по id, не по пути** — подставить чужой путь неоткуда.

Отдельно закрыт случай, который иначе бы пропустили: `setMessageAttachments`
тоже персистит, иначе всё, что сделал агент, терялось при перезапуске, а всё,
что приложил пользователь, сохранялось.

**Facts / Decisions / Conflicts** уже были тремя отдельными сущностями с
отдельными моделями данных и компонентами; старые prose-текстареа из интерфейса
убраны ранее. Дополнительной работы не потребовалось.

**Тесты**: Rust 78 → 85, frontend 131 → 149.

**Гейты**: typecheck OK, Vitest 149/149, Rust 85/85,
`cargo check --all-targets` 0 warnings, build OK, smoke 5/5.

**Ограничения**

- Миграция v2 **прорепетирована на копии реальной базы владельца** и прошла:
  v1 → v2, 24 проекта, 495 фактов, 5 сессий, 7 сообщений — до и после
  одинаково, `integrity` чистая. Репетиция доступна как opt-in тест:
  `MAGNETAR_MIGRATE_FIXTURE=<копия> cargo test an_existing_database`.

  Попутно репетиция подтвердила решение о `VACUUM INTO`: первая копия была
  сделана обычным `cp` и содержала 22 проекта, 434 факта и **ноль сообщений**,
  тогда как `VACUUM INTO` той же базы дал 24, 495 и 7. Разница лежала в WAL.
  То есть наивное копирование потеряло бы два проекта, 61 факт и всю переписку —
  ровно та ошибка, которую `backup_to` не делает.
- Импорт памяти и восстановление вложений не проверялись на живом приложении.
- `messages` не имеет FK на `sessions` — вне области среза, кандидат на v4.

**Следующий шаг — ШАГ 4**: базовый профессиональный редактор (split panes,
pinned tabs, regex-поиск с отменой и бюджетом, замена по workspace, breadcrumbs
и outline без LSP, multi-root или задокументированный отказ, VS Code-совместимые
клавиши, импорт/экспорт настроек).

### Запись 127 — 2026-08-27 — Claude — ШАГ 4 ЗАВЕРШЁН

**Поиск (`b8d690a`).** Был обход подстрокой без учёта регистра, с жёстким
пределом в 100 и без возможности остановить. На большом репозитории это даёт
один из двух плохих исходов: вернуться раньше, чем дошли до интересной
половины, или заставить ждать без выхода. Ни то ни другое поиском не является.

`src-tauri/src/search.rs`: регулярка или литерал, регистр и целое слово, бюджет
результатов, дедлайн и отмена **по id** — по id потому, что иначе быстро
печатающий пользователь отменял бы тот поиск, которого ждёт, вместе с
брошенным. Отмена и часы проверяются на каждой записи каталога, поэтому папка
из ста тысяч сгенерированных файлов останавливается по просьбе.

Каждый прогон сообщает, чем именно был обрезан: бюджетом, временем или
пользователем. Это разные вещи для читающего — сузить запрос, подождать, или
ничего.

Литеральный поиск идёт через тот же движок регулярок, экранированным: одна
ветка сопоставления означает, что колонка совпадения считается одинаково в
обоих режимах, и «целое слово» работает для литерала без второй реализации.
Колонка — это то, что позволяет панели подсветить совпадение, а не оставлять
читателя искать его заново.

Замена переведена на тот же движок. Раньше она шла от ранжированного BM25,
который матчит **слова** — правильно для «где это живёт», неправильно для
замены. Теперь список, показанный перед заменой, и есть тот, который заменят.
Литеральная замена идёт через функцию, чтобы доллар, введённый пользователем,
остался долларом, а не стал ссылкой на группу. Подсчёт защищён от
zero-width-шаблонов вроде `x*`, которые вешают наивный цикл `exec`.

Тест поймал `bundle.min.js` в выдаче: расширение `js`, а по сути генерат.
Теперь такие узнаются по имени, а не только по расширению.

**Вкладки и автосохранение (`ede4819`).** Закрепление: закреплённые идут первыми
и переживают «закрыть все». Автосохранение с задержкой, по умолчанию выключено —
включить его молча значит, что редактор начинает менять файлы на диске без
просьбы, а для инструмента с агентом это не то допущение, которое можно делать
за пользователя.

**Клавиши и настройки (`2f16bd0`).** Сочетания стали таблицей, и умолчания несут
чорд VS Code рядом с собственным там, где они расходятся. Палитра — главный
случай: в VS Code это ⌘⇧P, в Magnetar всегда было ⌘K. Работают оба; убрать одно
ради стройности таблицы значило бы выбрать аккуратность вместо человека,
жмущего клавишу.

`keybindings.json` импортируется вместе с комментариями и висячими запятыми —
отказать файлу, который у людей реально есть, значит сделать функцию
бессмысленной. Что не удалось перенести, называется поимённо: придуманная
привязка к тому, чего Magnetar не умеет, была бы сочетанием, которое молча
ничего не делает.

Экспорт/импорт настроек несёт вид приложения и ничего секретного. Настройки,
называющие что-то на этой машине (id подключения в `memoryModel`), не переносятся.

**Раскол области и крошки (этот коммит).** Одна вторая панель, не произвольное
дерево групп: два файла рядом — тест и то, что он тестирует; интерфейс и
реализация — это то, за чем реально тянутся, и раскладка остаётся такой, которую
человек удерживает в голове. Панель — тот же редактор, а не урезанный просмотр:
второй файл, на который можно смотреть, но нельзя починить, хуже, чем никакого.

Попутно найдено и исправлено: буфер грузился только для активной вкладки, и
вторая панель отрисовала бы пустоту, выглядящую как пустой файл.

Крошки — пока по пути. Символьные требуют парсера, работающего без языкового
сервера, — это Шаг 5.

**Multi-root — решение, а не пропуск.** Withdrawn с обоснованием в
`docs/IMPLEMENTATION_PLAN.md`: три вещи в Magnetar определены через «проект» —
containment, доверие и индекс, — и multi-root превращает каждую в вопрос с
несколькими ответами. Интересны как раз случаи, где ответы расходятся. Цена
отказа узкая: второй проект — второе окно. `IDE-07` снят, потому что приёмочный
тест на то, что сознательно не делается, — это вечный ложный провал.

**Тесты**: Rust 85 → 98, frontend 149 → 174.

**Гейты**: typecheck OK, Vitest 174/174, Rust 98/98,
`cargo check --all-targets` 0 warnings, build OK, smoke 5/5.

**Ограничения**

- Ни новая панель поиска, ни раскол, ни закрепление вкладок **не проверялись на
  живом приложении** — всё это требует открытой папки, а вне Tauri её нет.
  Проверено типами, тестами логики и сборкой.
- Символьные крошки, outline и workspace symbols — Шаг 5.
- Раскол не восстанавливает позицию прокрутки второй панели между сессиями
  (сохраняется только путь).

**Следующий шаг — ШАГ 5**: LSP и парсер-слой. Переход к определению, ссылки,
переименование, hover и автодополнение уже подключены для четырёх языков.
Нет: форматирования и format-on-save, organize imports, code actions, document
и workspace symbols, semantic tokens, onboarding при отсутствии сервера,
restart/reconnect и парсера, дающего структуру без сервера вообще.

### Запись 128 — 2026-08-27 — Claude — ШАГ 5 ЗАВЕРШЁН

**Структура файла без сервера (`577233e`).** Главное решение шага. Outline и
крошки требуют знать, что в файле. Сервер знает точно — и не установлен, или
ещё стартует, или для этого языка его нет, чаще, чем наоборот. Панель
структуры, пустая до `npm install -g`, — это панель, которую никто не видит.

`src/lib/outline.ts` читает объявления с начала строк и вкладывает по отступу:
TS/JS, Rust, Python, Go, shell, Markdown, JSON, YAML. Это эвристика, и так и
написано: она пропустит функцию, присвоенную внутри вызова, и радостно покажет
что-то из блока комментариев. Взамен ничего не стоит, работает с первого
нажатия и покрывает все языки, а не четыре.

Два пробела вытащили тесты. Методы класса не имеют ключевого слова, а методы —
это и есть основное содержимое outline; распознаются по форме: отступ, сигнатура
вызова, открывающая скобка в конце строки. Это, в свою очередь, начало ловить
`if (x) {` как символ с именем «if» — слова управления потоком исключены.

**Tree-sitter отложен, а не забыт.** WASM-рантайм плюс грамматика на язык —
пара мегабайт ради представления, которое и так верно в девяти случаях из
десяти. Сделка окупится, когда понадобится настоящее дерево разбора, — это
inline completion (Шаг 11).

**Форматирование и действия (`f851781`).** Пять запросов, которых мост не делал:
formatting, rangeFormatting, codeAction, organize imports, documentSymbol.

Форматтер берётся у сервера, а не у нас: это решение проекта — prettier,
rustfmt, black, gofmt, — и сервер знает, какое. Format-on-save выполняется
**до** записи и читает результат обратно из модели, иначе диск и редактор
расходились бы до следующего нажатия. Нет форматтера или он упал — файл
сохраняется как написан: отказ сохранить из-за форматирования потерял бы правку.

Из code actions предлагаются только те, что несут правки. Выполнить произвольную
команду сервера — разрешение другого рода, чем применить текстовую правку, и
этот шаг его не выдаёт.

**Состояние серверов (`9a469fa`).** Видимых состояний было два: «работает» и
«молча не работает», и из редактора они неотличимы. Теперь сервер сообщает, где
он: стартует, готов, не установлен (со строкой установки), перезапускается,
или сдался. `gaveUp` отделён от `failed` намеренно: первое — «умер и попробует
снова», второе — «умирал подряд, нужно что-то менять», и только второе стоит
того, чтобы отвлекать человека. Перезапуск ручной по той же причине.

**Символы по проекту (`a7fbf01`).** ⌘T спрашивает **все** живые серверы, а не
сервер текущего файла: проект редко на одном языке, и поиск, находящий символы
только в языке открытой вкладки, хуже отсутствия поиска — нельзя понять, символа
нет или он в файле, на который ты не смотришь. Незапущенные серверы не
поднимаются: человек, печатающий в палитре, не просил индексировать Rust.

**Semantic tokens (`b1a085b`).** Токенизатор Monaco работает по шаблонам и не
отличит тип от переменной с большой буквы. Легенда приходит один раз в ответе
на initialize — раньше этот ответ выбрасывался целиком. Каждый сервер нумерует
по-своему, поэтому токены переотображаются на свой список; неизвестный тип
**отбрасывается**, а не приближается: переменная, покрашенная как тип, хуже
непокрашенной.

**Языки из списка**: JSON, CSS, HTML имеют настоящие воркеры Monaco (валидация,
автодополнение, форматирование); YAML, Markdown, Bash — подсветка плюс outline
из эвристики. Валидации YAML нет — это отдельная зависимость (`monaco-yaml`),
и добавлять её ради одного языка в этом шаге не стал.

**Тесты**: frontend 174 → 194.

**Гейты**: typecheck OK, Vitest 194/194, Rust 98/98,
`cargo check --all-targets` 0 warnings, build OK, smoke 5/5.

**Ограничения**

- **Ничего из LSP не проверено на живом приложении.** Форматирование, code
  actions, символы, semantic tokens и панель состояния требуют запущенного
  сервера и открытой папки. Покрыты чистые функции (эвристика outline,
  переотображение токенов) и типы; сквозной проверки нет.
- Валидации YAML нет.
- Semantic tokens запрашиваются целиком (`full`), без инкрементальных дельт:
  на очень больших файлах это дороже, чем нужно.

**Следующий шаг — ШАГ 6**, самый крупный из оставшихся: Git за пределами
проброса (hunks, ветки, merge, rebase, stash, cherry-pick, разрешение
конфликтов, blame, история, remotes, подпись коммитов), задачи из
package.json/Cargo/Make/pyproject/just, запуск одного теста и набора,
Test Explorer и DAP-отладчик начиная с Node и Python.

### Запись 129 — 2026-08-27 — Claude — ШАГ 6 ЗАВЕРШЁН

Самый крупный шаг. Три больших блока: Git, задачи/тесты, отладчик.

**Git-слой (`a058119`).** Панель раньше строила массивы аргументов git по месту
и читала сырой вывод там же — годится для status, не масштабируется. `git.ts`
держит операции, а парсинг — чистые функции над сырым выводом, с тестами. Две
вещи, которые тесты вытащили: ветку, буквально названную со `*`, `branch -a`
прочитал бы как текущую (поэтому формат с NUL-разделителями и полем `%(HEAD)`);
состояние операции читается из refs (MERGE_HEAD/CHERRY_PICK_HEAD/REBASE_HEAD),
а не из .git на диске.

**Hunk-staging (`817fa1d`).** `git add -p` без терминального промпта: берём дифф
файла, выделяем один hunk, строим патч из него плюс заголовки. Построение —
чистые функции с тестами (патч не тянет следующий hunk, кончается переводом
строки, который git требует). Для stdin добавлена отдельная команда `git_apply`
— общий `git_exec` не учат писать stdin. Discard без undo, поэтому подтверждает.

**Ветки, stash, конфликты (`e58f33b`).** Меню веток: switch/create/delete,
checkout удалённой как локальной, merge/rebase между ветками. Главное — баннер
конфликта: разрешённый merge/rebase — момент, когда люди уходят в терминал,
потому что GUI молчит. `repoState` читает операцию и список неслитых файлов,
баннер даёт продолжить (только когда чисто) или отменить. У merge нет
--continue, поэтому «продолжить» коммитит разрешённый индекс.

**Blame/история/подпись (`02b3e35`).** Подпись определяется, а не пробуется:
пробный коммит был бы нечестен. Blame — только текущая строка (GitLens-стиль),
включается по кнопке. all-zero sha = «не закоммичено». История файла в крошках,
с относительным временем (своя тестируемая функция, будущий timestamp не даёт
отрицательный возраст).

**Задачи (`e7f9b11`).** Панель находит команды из package.json/Cargo/Makefile/
pyproject/justfile. Парсеры — чистые функции с тестами (`:=` не цель, тело
justfile-рецепта не рецепт, lockfile решает pnpm против npm). Запуск — в
терминал (там видно и можно прервать), через новый `runInTerminal`. Селекторы
одного теста — по фреймворку, null где чистого нет.

**Отладчик (`14827dd`).** DAP — тот же фрейминг, что LSP, поэтому транспорт
переиспользован. Клиент добавляет корреляцию ответов по seq — часть, которая
ломается незаметно (ответ не тому запросу = не те переменные), поэтому там и
тесты. Сессия гоняет handshake в порядке спеки (initialize → точки останова +
configurationDone в окне события `initialized` → launch). Точки останова в
gutter'е и переживают сессию (ставятся при чтении, персистятся); стек,
переменные, watches, консоль — процесс, не сохраняются. Python первоклассно
через debugpy; Node с onboarding-хинтом (js-debug — большой отдельный пакет,
не встроен). Готовность адаптера — поле, а не догадка.

**Remotes (этот коммит).** Добавление/удаление в меню веток.

**Осознанно не сделано, зафиксировано в матрице**

1. Test Explorer с деревом по отдельным тест-кейсам: панель задач группирует
   тесты и запускает названный, но перечисление кейсов — это либо прогон
   набора, либо парсинг тест-файлов, отдельный проект.
2. Node-отладчик — только onboarding-хинт, не рабочий launch.

**Тесты**: Rust 98 (без изменений — DAP-транспорт переиспользует lsp.rs),
frontend 194 → 235.

**Гейты**: typecheck OK, Vitest 235/235, Rust 98/98,
`cargo check --all-targets` 0 warnings, build OK, smoke 5/5.

**ОГРАНИЧЕНИЯ — важно**

Ничего из Шага 6 не проверено на живом приложении: git-операции требуют
репозитория и открытой папки, отладчик — установленного debugpy и запущенного
Tauri, задачи — терминала. Покрыты чистые функции (все парсеры, построение
патча, корреляция DAP) и типы. Сквозной проверки нет — это самый большой объём
непроверенного руками кода за все шаги. Первый запуск подписанной сборки должен
пройти: hunk-staging, переключение веток, баннер конфликта (сделать
искусственный конфликт), запуск задачи в терминале, и отладку .py файла с
точкой останова (нужен `pip install debugpy`).

**Следующий — ШАГ 7**: индекс и производительность. Снять лимит 5000, сделать
persistent и инкрементальным с file watcher, учитывать .gitignore, перейти на
persistent BM25/FTS, зафиксировать метрики масштаба (1k/10k/50k/100k).

### Запись 130 — 2026-08-27 — Claude — ШАГ 7 ЗАВЕРШЁН

**Индекс на FTS5 (`ae16545`).** Старый индекс: в памяти, пересборка с нуля при
смене root, лимит 5000 файлов — три провала сразу. Теперь одна SQLite-база на
воркспейс, на диске, инкрементально по size+mtime, без лимита (тест находит все
6000 файлов, где старый останавливался на 5000). Ранжирование — FTS5 bm25.
Обход через ripgrep-крейт `ignore`: .gitignore/.ignore честно применяются, в том
числе до `git init` (`require_git(false)`). File watcher (`notify`) следит за
деревом и пере-синхронизирует после затишья, debounce 400мс — `git checkout`
на тысячу файлов даёт один sync. Удаление проекта чистит файл индекса.

Баг, который вытащил тест: каталог индекса лежал внутри обходимой папки, индекс
индексировал свои же `.sqlite`, WAL-запись выглядела изменением. В проде индекс
в app_data, отдельно; в тесте — sibling-каталог.

**Нарезка больших файлов (`e96252a`).** `read_file` с окном теперь читает
построчно и останавливается на конце окна, а не грузит файл целиком. Строки
10-30 из 500МБ-лога стоят тридцать строк. Пункт из SECURITY.md закрыт.

**Ленивая загрузка сессий (`e96252a`).** Hydrate читал все сообщения всех
сессий. Теперь только метаданные; сообщения приходят при первом открытии
(`ensureMessages`, идемпотентно). Сто старых чатов при запуске = сообщения
одного, а не ста.

**Дерево файлов** уже было ленивым (children грузятся при раскрытии) — доп.
работы не потребовалось.

**Bounded-очередь (этот коммит).** Summary/extraction/verify гонялись как голые
промисы. Теперь очередь на 2 параллельных, по приоритету, с отбрасыванием
дубля по ключу. Summary — normal, extraction — low.

**Покрытие индекса** — число в статус-баре (файлов, сколько пропущено).

**Тесты**: Rust 94 → 96 (index переписан: было 12 старых тестов, стало 7 новых;
+2 на нарезку файла), frontend 235 → 240.

**Гейты**: typecheck OK, Vitest 240/240, Rust 96/96,
`cargo check --all-targets` 0 warnings, build OK, smoke 5/5. Smoke теперь ~15с
из-за теста на 6000 файлов.

**ОГРАНИЧЕНИЯ**

- Метрики масштаба в QUALITY_GATES — цели, не измерения. 50k/100k файлов
  реально не прогонялись; это бенч Шага 14 на подписанной сборке.
- File watcher не проверен на живом приложении (нужен Tauri). Логика sync
  (инкремент/удаление/ignore) покрыта Rust-тестами; сам watcher — нет.
- Индекс не проверен на реальном большом репозитории.

**Следующий — ШАГ 8**: agent runtime в headless-ядро — durable run_id, event
log, pause/resume, отмена до провайдера и группы процессов, retry/backoff,
circuit breaker, компактификация контекста, бюджеты токенов и денег, resume
после перезапуска.

### Запись 131 — 2026-08-31 — Antigravity — Реализация Universal Prompt Builder (/prompt)

Реализована фича генерации системных промптов для генеративных моделей (Universal Prompt Builder).
Вместо отдельной дорожки или сложного UI-диалога применена интеграция в Composer через автокомплит слэш-команд.

**Что сделано:**
- **Модели в автокомплите (`mentions.ts`)**: Добавлен список моделей (`Kling`, `Seedance`, `Flux` и т.д.) прямо в `SLASH_COMMANDS`, что позволяет вводить `/prompt Kling <текст>` инлайн.
- **Инжект системного промпта (`expandSlash`)**: При парсинге `/prompt <Модель>` запрос оборачивается в сильную инструкцию для LLM и требует вернуть текст внутри тега `<MagnetarPrompt>`. Это работает в обеих дорожках (`chat` и `agent`), так как логика `slashInstruction` была перенесена в `runSend` (раньше слэш-команды в `ChatView` работали только в `runAgentPath`).
- **Рендеринг карточки (`Message.tsx`)**: Регулярки `extractMagnetarPrompt` вырезают сгенерированный блок и отображают его под основным текстом (который рендерит `ReactMarkdown`) с кнопками `Скопировать` и `Отправить в Studio`.
- **Zustand Store (`agentRun.ts`, `StudioView.tsx`)**: Добавлен `pendingStudioPrompt` для программного переноса текста в форму генерации. При нажатии «Отправить в Studio» приложение переключается на вкладку StudioView с предзаполненным инпутом.
- **Локализация (`i18n.ts`)**: Добавлены ключи `promptGenerated` и `promptSendToStudio` для `ru`, `en`, `es`.

**Ограничения/Риски:**
- Отсутствует жесткая блокировка "слепых" моделей. Если текущая модель в чате не имеет Vision, а пользователь прикрепил картинку, запрос уйдет, и ошибка будет зависеть от ответа провайдера.
- Список генеративных моделей для `/prompt` захардкожен в `mentions.ts` (выжимка из `GEN_PROVIDERS`), чтобы не усложнять автокомплит длинными ID.

### Запись 132 — 2026-08-31 — Antigravity — Упрощение автокомплита `/prompt`

По просьбе пользователя базовые команды `/промт` и `/prompt` (без указания модели), а также русские алиасы `/промт <Модель>` были полностью удалены из выпадающего меню `SLASH_COMMANDS` в `mentions.ts`. 

Это сделано для того, чтобы не засорять меню. Теперь в нём остаются только конкретные английские команды (`/prompt Kling`, `/prompt Seedance` и т.д.), которые сразу подставляют нужный генератор. Логика `expandSlash` также обновлена под перехват исключительно `/prompt`.

---

**Шаг 133: Кнопки создания файлов и папок в UI проводника (MVP)**
- Добавлен отсутствующий функционал создания файлов и папок в интерфейсе левой панели `ExplorerPanel`.
- В заголовок добавлены иконки `FilePlus2` (Новый файл) и `FolderPlus` (Новая папка).
- Реализовано отображение inline-поля ввода (`input`) имени файла/папки с обработкой `Enter` и `Escape`.
- Добавлен вызов Rust-команды `toolCreateDir` (создан бэкенд для `create_dir` в `tools.rs` и `commands.rs`).
- Используется существующая команда `toolWriteFile` для создания пустого файла.
- Исправлена недоделка, мешавшая полноценно начинать проекты в пустых папках без Агента.

## Сессия (Генерация через OpenRouter/TokenRouter)

### Что сделано
- **Генерация через `chat-proxy`**: Для провайдеров типа OpenRouter и TokenRouter, которые не поддерживают стандартные эндпоинты `images/generations`, реализована прокси-генерация через текстовый `v1/chat/completions`. Бэкенд (`generate_chat_proxy`) отправляет запрос к текстовому API и извлекает URL-адреса сгенерированных изображений из ответа модели, а фронтенд передает стратегию `"chat-proxy"`.
- **Динамический список моделей в Студии Генерации**: Раньше список генеративных моделей был жестко зашит (например, только `openai/dall-e-3`). Теперь для `chat-proxy` провайдеров `StudioView` динамически запрашивает `/models` из API (как в текстовом чате), что позволяет выбрать любую доступную мультимодальную модель, способную отдавать изображения.
- **Исправление багов интерфейса**:
  - Выпадающий список выбора модели (`Select.tsx`) обрезался из-за `overflow-auto` в сайдбаре студии. Позиционирование изменено с `right-0 w-max` на `left-0 w-full` для корректного отображения длинного списка (сотни моделей).
  - В настройках исправлен подсчет доступных моделей для `chat-proxy` провайдеров.
...
**Решение (Hamid, 2026-09-01):** простая студия генерации — это **финальный V1**.
Графовый Workflow Engine снят с плана, строить его не нужно. Пометки о расхождении
в доках закрыты этим решением. Дальше — Фаза 1 (надёжность агента: durable runtime
+ чекпоинты/откат).

### Запись 135 — 2026-09-01 — Claude Opus 4.8 — Фаза 1: надёжность агента (durable runs)

Продолжение работы над видением (после Фазы 0). Цель фазы — сделать прогоны
агента durable: переживают перезапуск, с бюджетами и откатом. Сделано мелкими
безопасными шагами, каждый — отдельный коммит, все проверки зелёные.

**Шаг 1 (`adf1bf6`) — слой данных.** SQLite схема v4: таблицы `agent_runs`
(канон прогона: статус, шаги, токены, бюджеты, ошибка) и `agent_events`
(append-only трейс: model_turn/tool_call/tool_result/text/error/checkpoint).
Rust CRUD в `workspace.rs` + Tauri-команды + обёртки `db.ts`. Чисто добавляющий.

**Шаг 2 (`6d59eb2`) — запись из живого цикла.** Новый `src/lib/agentRunLog.ts`
оборачивает `AgentHandlers` (сам `agent.ts` не тронут): на старте пишет строку
`running`, на каждый tool/usage — событие, в конце — done/cancelled/error. Все
записи fire-and-forget (в веб-превью нет Tauri — падать нельзя). Подключено в
`ChatView`.

**Шаг 3 (`e82beec`) — бюджет токенов.** Новый pref `agentMaxTokens` (слайдер в
настройках, 0 = выкл, дефолт 400k). runLog отдаёт `overBudget()`, оба цикла
агента проверяют его между шагами и останавливаются с объяснимой причиной.

**Шаг 4 (`242a1e8`) — восстановление после перезапуска.** `db::reconcile_agent_runs`
(тестируемая, на сыром conn) помечает застрявшие `running`/`paused` как
`interrupted` при старте (в `hydrate`). Прерванные показываются баннером
(ru/en/es). Честная отмена (Stop → halt loop + abort сети + `toolKillBash` группы
процессов) **уже была** — это закрыло AGENT-02.

**Шаг 5 (`a3c446e`) — чекпоинт прогона.** Каждая правка файла помечается `runId`
(слайс проставляет из `activeRunId` — `agent.ts` не тронут). Панель «Изменения»
группирует правки прогона и даёт «Откатить весь прогон». Пофайловый undo остался.

**Итог:** AGENT-01 ✅ (durable runs+трейс, reconcile), AGENT-02 ✅ (отмена+retry),
AGENT-03 ◐ (бюджет токенов есть; стоимость/компакция — нет), CHANGE-01 ✅
(откат целиком+пофайлово; по-ханкам — нет). Проверки: tsc чисто, Vitest 244/244,
Rust 99/99, i18n полный.

**Осознанно НЕ сделано:** headless Rust-ядро цикла (цикл остаётся во фронте — там
он работает, переписывание рисковало бы рабочим приложением ради невидимой
пользе), per-token стоимость, компакция контекста, откат по ханкам. Открыто, если
понадобится демон фоновых прогонов с закрытым приложением.

**Дальше:** Step 10 (MCP) или другой пункт по решению Hamid.

### Запись 136 — 2026-09-01 — Claude Opus 4.8 — Фаза 2: паритет IDE

Автономная работа (Hamid: пересборка только в конце Фазы 4). Фаза 2 — три пункта.

**Inline completion (`2645d51`) — СДЕЛАНО.** Новый `src/lib/inlineCompletion.ts` —
провайдер ghost-text для Monaco, регистрируется в `onMount` (идемпотентно). Модель
берётся из памяти (или чата). Три ограничения стоимости: капнутый контекст
(2000/500 символов до/после курсора), дебаунс 300мс, отмена устаревших запросов
токеном Monaco. Приём (Tab/слово/строка) — встроенными командами Monaco. Опция
`prefs.inlineCompletion` (тумблер в настройках ru/en/es), **по умолчанию выкл**
(тратит токены, нужна модель). Двойной гейт: опция редактора + проверка pref.
Тесты на `clean()` (снятие ```-ограждений).

**LSP TS-worker — уже сделано by design.** Мост LSP уже настроен на
`typescript-language-server` для TS и JS (семантика: определения/ссылки/
переименование/диагностика когда сервер установлен). Встроенный TS-воркер Monaco
оставлен только для синтаксиса + однофайловый автокомплит-«пол», семантическая
валидация выключена (иначе ложные «не найдено»). Гибрид лучше полной замены —
сохраняет офлайн-пол без сервера. Пробелом не считаю.

**MCP — ОТЛОЖЕН с обоснованием.** MCP-клиент протокол-зависим (spawn, initialize,
tools/list, tools/call по newline-JSON-RPC) и его нельзя сделать ответственно без
живых MCP-серверов и итераций в приложении (deny-by-default flow). Строить вслепую
в окне без пересборки = большой непроверяемый слой в рабочем приложении. План на
будущее записан в `IMPLEMENTATION_PLAN.md`: `mcp.rs` (карта хэндлов как у LSP),
durable `mcp_servers`, команды `mcp_list_tools`/`mcp_call_tool`, интеграция в
агента с namespace `mcp__<server>__<tool>`, confirm по умолчанию, аудит. Пока MCP
не существует — ничего не полу-построено.

**Проверки:** tsc чисто, Vitest 248/248, i18n полный. Дальше — Фаза 3 (студия/видение).

### Запись 137 — 2026-09-01 — Claude Opus 4.8 — Фаза 3: студия/видение

**Персист галереи (`c33fe9a`) — СДЕЛАНО.** Результаты студии жили только в
модульном кэше — перезапуск терял галерею. Теперь каждый ассет пишется в durable
таблицу `generations` (уже существовала, не использовалась): Rust CRUD
save/list/delete_generation + команды + обёртки `db.ts`. StudioView восстанавливает
последние генерации при первом открытии в сессии (только если транскрипт пуст — не
затирая живой прогон). Удаление восстановленного — навсегда. Записи fire-and-forget.

**`@image1` референсы — уже работают** (paperclip → refs → image-модель; заметка
«disabled» в NEXT_SESSION_PROMPT устарела). Промт-мейкер в чате отложен Hamid'ом.

**Не сделано:** больше провайдеров (Replicate/Imagen/ElevenLabs) — нужны живые
ключи для проверки wire-форматов, вслепую не добавляю.

Проверки: tsc чисто, Vitest 248/248, Rust 99/99. Дальше — Фаза 4 (bench поиска).

### Запись 138 — 2026-09-01 — Claude Opus 4.8 — Фаза 4: масштаб поиска (bench + фикс)

**Бенчмарк (`aefda7d`).** Добавил `bench_index_scale` в `index.rs` (`#[ignore]`,
запуск `cargo test -- --ignored --nocapture bench_index_scale`, число файлов через
`MAGNETAR_BENCH_FILES`). Генерит N файлов в 200 папках, замеряет sync + запросы.

**Найден и исправлен O(n²) баг.** Запуск на 50k вскрыл: **начальный sync ≈ 621 с
(~10 мин)**. Причина — `docs` это FTS5, а `DELETE ... WHERE path=?` по FTS не может
использовать индекс и сканирует; при начальном билде все файлы новые, но DELETE
делался перед каждым INSERT → скан растущего индекса на каждый файл = квадратично.
Фикс: DELETE только когда путь реально был в `stored` (реальная замена).

**Измерено (50k, debug, dev-Mac):** initial sync **621185мс → 5611мс (~110×)**;
no-op re-sync ~1.9с; 2 запроса ~121мс (последние два были и так в бюджете).
Корректность цела (99 тестов, no-cap/search). QUALITY_GATES обновлён реальными
числами (цели → измерения). Release-прогон на реальном 100k-репо в подписанном
приложении остаётся бенчем Шага 14, но алгоритмическая проблема устранена.

**Фаза 4 завершена — по договорённости пересобираю приложение.** Дальше — Фаза 5
(релиз) требует Apple Developer ($99) — действие Hamid.
