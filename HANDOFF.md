...
- **Фронтенд (TS/React):**
  - **`Composer.tsx`**: Добавлена иконка `Paperclip` для загрузки локальных изображений. Поддержка Drag-and-Drop (через paste event) для вставки картинок из буфера обмена (сохраняются как base64/dataURLs). Добавлена галерея превью картинок под инпутом.
...
<!-- Следующий ассистент: добавь «Запись 14 — дата — модель — тема» здесь. -->
